...
- **Main-to-renderer profile transport**: the trusted Electron bridge path that
  publishes only the capability-scoped Joplin database and resource binding
  after vault unlock. Renderer startup fails closed when that binding is
  absent; no Local Vault Key or derived key crosses this seam.
